...
**This is where the personality code for SparkOS exists.**

For the entire OS, check out the [spark-os](https://github.com/LaziTeto/spark-os) repo. This repo, `spark-os-victor`, is a submodule of that, and just builds the /anki folder which goes into that OS. However, `spark-os-victor` can still be built standalone and deployed to a robot which is running a good base OTA. This is recommended for developers.

Most changes happen in this repo. If one wants to, for instance, add a new feature; this is where they'd do it.

If you want to add a program to the OS, do that in [spark-os](https://github.com/LaziTeto/spark-os).
...
SparkOS serves as a stable, up-to-date, easily-buildable base for CFW, and is a continuation of Anki's work.
...
`spark-os-victor` can be built standalone on most Linux distros (arm64 or amd64) and on macOS (arm64/M-series).
...
```
cd ~
git clone --recurse-submodules https://github.com/LaziTeto/spark-os-victor/
cd spark-os-victor
```
...
1. Install SparkOS on your robot.
...
`spark-os-victor` can be run in WeBots. A mostly fully-featured Vector experience without needing a real robot to deploy to.
